...
- 主键约束
	- 它能够**唯一确定**一张表中的一条记录，增加主键约束之后，就可以使得**字段不重复而且不为空** 。
- 自增约束
...
- 非空约束
	- 修饰的字段不能为空NULL
- 默认约束
	- 当我们插入字段值的时候，如果没有传值，就会使用默认值，传了值就不会使用默认值
- 外键约束
	- 涉及到两个表：主表，副表
	- 主表中没有的数据,在附表中,是不可以使用的.
	- 主表中记录的数据现在正在被附表所引用,那么主表中正在被引用的数据不可以被删除
	- 若要想删除,先将附表中的数据删除再删除主表数据
...
* **第二范式 2NF**
    *  **必须满足第一范式，需要确保数据库表中的每一列都和主键相关，而不能只与主键的某一部分相关（主要针对联合主键而言）；**或者说每一个非主属性都完全函数依赖与任何一个主键
...
### 聚集索引和非聚集索引
- **聚集索引**：数据行的物理顺序与列值的**顺序相同**，如果我们查询id比较靠后的数据，那么这行数据的地址在磁盘中的物理地址也会比较靠后。而且由于物理排列方式与聚集索引的顺序相同，所以也就只能建立一个聚集索引了。
- **非聚集索引**：索引中索引的逻辑顺序与磁盘上行的物理存储顺序不同，一个表中可以拥有多个非聚集索引。
...
- 假如每个节点可以存 B 个key，那么高度为 N 的B+树有 B^N个叶子节点，那么在数据量为 n 的条件下，树的高度大约为 log(n)/log(B)。
- 一般innoDB的pageSize为16k，非叶节点存放key和pointer，假设key大小为8字节，pointer大小为4字节，那么一个page（磁盘块）可以存 16k/(8+4) 个 key，假设一个叶子节点上的数据大小为100字节，那么一个节点可以存16k/100个数据，那么一个高度为3的B+树，就可以存放 16k/(8+4) * 16k/(8+4) * 16k/100 = 2000多w 条数据。

### 强制索引

- `force index(index_name);`   强制使用某个索引

- `ignoreindex(index_name);`   禁止某个索引
...
* 外键
    * InnoDB支持，MyISAM不支持 
* 锁
    * InnoDB支持行锁，表锁。行锁可以提高多用户并发操作，但InnoDB的行锁，**主键和唯一索引才是行锁，当“值重复率”低时，甚至接近主键或者唯一索引的效果，“普通索引”依然是行锁；当“值重复率”高时，MySQL 不会把这个“普通索引”当做索引，即造成了一个没有索引的 SQL，此时引发表锁**。
    * MyISAM支持表锁
